...
EPYDOC_CMD=c:\python26\python.exe c:\python26\scripts\epydoc.py
...
# Install the module
install:
	$(PYTHON_CMD) -O setup.py install


# Build the tools
# Don't use the -OO switch here, it breaks pdebug.py
py2exe:
	$(PYTHON_CMD) -O setup.py py2exe
...
sdist:
	$(PYTHON_CMD) -O setup.py sdist $(SDIST_OPT)
...
# Build the Windows installer package
wininst:
	$(PYTHON_CMD) -O setup.py bdist_wininst $(BDIST_UAC)

# Build the Windows MSI installer package
msi:
	$(PYTHON_CMD) -O setup.py bdist_msi
