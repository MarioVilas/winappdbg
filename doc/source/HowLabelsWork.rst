...
If debugging symbols are available, they are used automatically in addition to exported functions. To get the debugging symbols you need to first install the `Microsoft Debugging Tools <https://learn.microsoft.com/en-us/windows-hardware/drivers/debugger/>`_, and then either install the debugging symbols for your version of Windows or set up your system to connect to the `Microsoft Symbol Server <https://learn.microsoft.com/en-us/windows/win32/dxtecharts/debugging-with-symbols#using-the-microsoft-symbol-server>`_.
